...
The matching device `/system/lib64/libandroid_runtime.so` exports `android::register_android_content_XmlBlock(JNIEnv*)` and links `android::ResXMLTree` / `android::ResXMLParser`. Parsing the library image recovered all 22 expected name/signature/function triplets in order; the registration function uses 0x18-byte entries over a 0x210-byte table, exactly 22 entries. A runtime scanner was added that does not hard-code recovered function offsets: it locates loaded `libandroid_runtime.so` PT_LOAD segments, searches non-executable readable segments for the complete validated 22-entry `JNINativeMethod` sequence, and requires every recovered function pointer to land in an executable segment. API 36 provider-self validation now passes: `table_found=true`, `expected_method_count=22`, `method_count=22`, and `all_functions_in_executable_segment=true`. `register_symbol_resolved=false` under `RTLD_DEFAULT`, but table discovery is independent of that exported-symbol lookup and remained valid. At the Stage 43 boundary, existing-runtime-object `mNative`/`mParseState` capture and direct native ResXMLTree peer reconstruction were still unimplemented; Stage 47 later adds target-only Xposed capture for the hidden peer values while leaving direct arbitrary peer-memory reconstruction gated.

## Stage 44 - Isolated native XmlBlock/ResXMLTree replay

Built on the Stage 43 offset-free 22-entry `JNINativeMethod` discovery without attempting to bypass hidden Java fields. `memory.xml.block_probe` now accepts an explicit native-replay opt-in. It resolves the requested `resource_id` to its own APK-backed binary AXML entry, loads only those bounded bytes, calls the discovered platform `nativeCreate([BII)J`, `nativeCreateParseState(JI)J`, advances the resulting parser with `nativeNext(J)I`, records bounded line/attribute-count metadata, and always calls `nativeDestroyParseState(J)V` plus `nativeDestroy(J)V`. No arbitrary address or external byte buffer is accepted through this route, and the result explicitly reports `existing_peer_reconstruction=false`.

API 36 provider-self validation passed on `autocrack_runtime_probe.xml` (340 bytes): the platform native tree and parser were both created, the isolated native sequence returned `[2, 3, 1]` (`START_TAG`, `END_TAG`, `END_DOCUMENT`), `end_document_reached=true`, and both native peers completed their destroy path. The ordinary Java parser simultaneously exposed the expected document/start/end events. This proves the recovered runtime table can drive a real platform `ResXMLTree` lifecycle. At the Stage 44 boundary, extraction of an already-existing framework object's hidden `mNative` / `mParseState` was still unclaimed; Stage 47 later adds a target-only Xposed capture path for those peer handles.

## Stage 45 - Native XmlBlock event and attribute metadata

Extended the isolated native replay beyond lifecycle-only validation. The replay now calls the dynamically discovered `nativeGetNamespace`, `nativeGetName`, `nativeGetText`, `nativeGetSourceResId`, `nativeGetAttributeCount`, `nativeGetAttributeNamespace`, `nativeGetAttributeName`, `nativeGetAttributeResource`, `nativeGetAttributeDataType`, `nativeGetAttributeData`, `nativeGetAttributeStringValue`, `nativeGetIdAttribute`, `nativeGetClassAttribute`, and `nativeGetStyleAttribute` functions. Output is bounded by the same event/attribute limits as the Java parser preview and returns string-pool indices plus typed resource/data metadata; it still does not expose arbitrary native addresses.

API 36 provider-self validation passed on `autocrack_runtime_probe.xml`. The native START_TAG reported `name_index=3`, `source_res_id=0x7f010000`, two attributes, and typed metadata matching the Java-side resource: Android `name` resolved to resource ID `0x01010003`, data type `TYPE_STRING` with string index 4, while Android `version` resolved to `0x01010519`, data type `TYPE_INT_DEC`, data `1`. The replay again reached END_DOCUMENT and destroyed both peers. At the Stage 45 boundary, existing-object hidden-peer capture was still the remaining XmlBlock-native gap; Stage 47 later narrows that gap through Xposed hidden-field access in LSPosed-loaded target processes.

## Stage 46 - Native XmlBlock readable string-pool correlation

Extended the Stage 45 native metadata so its string-pool indices are no longer opaque. `memory.xml.block_probe` now reuses the same bounded APK-backed binary AXML bytes that fed the isolated platform replay, parses that AXML string pool, and resolves native event `namespace_index` / `name_index` / `text_index` plus attribute namespace/name/string-value indices into readable strings. The result records `string_pool_count` and `string_pool_resolution=file_backed_axml_string_pool`; invalid or absent indices remain null rather than being guessed.

API 36 provider-self validation passed with a six-entry string pool. The native START_TAG `name_index=3` resolved to `autocrack-probe`; attribute 0 resolved to Android namespace `http://schemas.android.com/apk/res/android`, name `name`, and string value `autocrack_runtime_probe`; attribute 1 resolved to the same Android namespace and name `version`, while correctly leaving its string value null because the native typed value is `TYPE_INT_DEC` data `1`. These readable native results match the ordinary Java `XmlResourceParser` preview, giving a direct cross-check between platform native `ResXMLTree` metadata and file-backed AXML decoding. This stage still did not claim extraction of an already-existing object's hidden native peer; the later Stage 47 Xposed path records peer handles only when the current runtime module is actually loaded into that target process.

## Stage 47 - Xposed existing XmlBlock native-peer capture path

Added a target-process-only `XmlBlockPeerRegistry` and Xposed bootstrap hook for `android.content.res.XmlBlock.newParser(int)`. When the current AutoCrack Runtime module is actually loaded by LSPosed into a target, the after-hook uses the real Xposed runtime's hidden-field access to read the returned parser's `mParseState` and the owning `XmlBlock.mNative`, then records those values in a weak registry keyed by the exact parser object. `memory.xml.block_probe` queries that registry for the parser it just opened and reports the capture strategy, native tree/parse-state values, source resource ID, and capture timestamp. This path does not guess ART Java-object offsets, write LSPosed configuration, or alter the native peers.

Provider-self validation on API 36 passed the required negative behavior: the module app is intentionally excluded from Xposed injection, so `xposed_existing_peer.captured=false` and `xposed_required=true` were returned while the Stage 44-46 native replay/string checks still passed. A dedicated target-app XML fixture (`0x7f020000`) built and installed successfully and the app cold-started. The first target attempt timed out before Stage 52 because Android package visibility blocked target-process provider discovery; after the Stage 52 request-broadcast fallback, Stage 53 device replay confirmed target capture with `captured=true`, nonzero `native_tree`, nonzero `parse_state`, and source resource ID `2130837504`. No LSPosed database mutation or forced reboot was performed.

## Stage 48 - Host validation sweep after XmlBlock peer capture

After committing Stage 47, a clean follow-up validation sweep confirmed the repository has no remaining uncommitted source or documentation changes. A focused Gradle rebuild of `:autocrack-runtime:assembleDebug` and `:runtime-inspector-test-app:assembleDebug` passed with the Xposed peer-capture code and capability-boundary wording in place. The build used an isolated `GRADLE_USER_HOME` to avoid the user's global `~/.gradle/init.gradle` repository injection, which conflicts with the project's `repositoriesMode` policy; the global Gradle conflict is environmental and not a source-code failure.

All five maintained Toolpack packages were rebuilt successfully: `ui-inspect`, `runtime-inspect`, `memory-dump`, `runtime-control`, and `simplehook`. The four shared-runtime Toolpack ZIP hashes from this sweep were `ui-inspect=2a4f951161899ee97e083be291ae21ebdb0c4ea5da838efa40bd3ec9d3913b47`, `runtime-inspect=b3519b35796eda480bec20820b4671956415d9fb66d37c606c798b955ddd325b`, `memory-dump=26537ea49f9372c5054375a13774d08575e382c4e869a96f30ec26395685333c`, and `runtime-control=502f63851d7548cbbb32d3d4df7bf9560038f6d03ff39da31b01a04a83dc39b2`. After the follow-up manifest-v2 upgrade, `simplehook` rebuilt with Toolpack SHA-256 `b29b064496b18145a5b48b8da6e899c4d0b2dcc31d89bc71e6158be0ed4bb9a3` and payload SHA-256 `52de7ef3f08bc698300d7a4abd9163450d0b50356e01ab29210d5f8d6dffaa7b`. CLI entrypoint smoke validation passed for `ui-inspect --help`, `runtime-inspect --help`, `memory-dump --help`, `runtime-control --help`, and `simplehook --help`.

ADB discovery confirmed `/Users/lucky/Library/Android/sdk/platform-tools/adb` and device `a4976c80` were visible in this continuation, but direct APK install / target `content call` validation scripts were blocked by the OpenAI tool safety layer before execution. Therefore this continuation records host/package/CLI validation as PASS while preserving target-process positive checks as not executed rather than pretending they passed.

## Stage 49 - SimpleHook Toolpack manifest schema v2 alignment
...
## Stage 51 - Device verification bundle refresh

Updated `scripts/prepare_device_verification_bundle.sh` so the handoff bundle now includes the current device-verification artifacts instead of only the older main App/static Toolpacks. The bundle now requires and copies `AutoCrackApp-debug.apk`, `AutoCrackRuntime-debug.apk`, `RuntimeInspectorTest-debug.apk`, and `SimpleHookTestApp-debug.apk`; rebuilds and copies the five maintained runtime Toolpacks plus their manifests; includes `FINAL_REPORT.md`, `TEST_MATRIX.md`, `CROSS_TOOL_INTEGRATION.md`, `validate_cross_tool_contract.py`, and the SimpleHook device regression script; and records the cross-tool host contract JSON in `logs/cross-tool-contract.json`.

Host validation ran `bash -n scripts/prepare_device_verification_bundle.sh` and generated a temporary bundle under `/tmp`, asserting the four APKs, five maintained Toolpack manifests, cross-tool contract JSON, and integration docs were present. This does not claim target-process execution; it makes the target-positive verification handoff reproducible once LSPosed has refreshed the current Runtime module path.

## Stage 52 - Runtime request broadcast fallback for target AppsFilter blocking

Device diagnostics showed that the scoped target process was actually attached to AutoCrack Runtime (`runtime_attached=true`, heartbeat recent, PID visible), so the remaining timeout was not caused by missing LSPosed scope. Logcat showed Android package visibility blocking target-process provider calls with `AppsFilter ... BLOCKED`, `No provider info for content provider com.luckylca.autocrack.runtime`, and `Unknown authority com.luckylca.autocrack.runtime`. The runtime request queue therefore needed a push path that does not depend on the target process discovering the module provider.

Added an API 34+ provider-to-target ordered broadcast request path: `runtime_submit` now stores the request, then sends `com.luckylca.autocrack.runtime.REQUEST` to the selected target package with sender identity sharing enabled. `InspectorEngine` registers a dynamic exported receiver in the injected target process, verifies that the sender UID belongs to `com.luckylca.autocrack.runtime`, and enqueues the same bounded request handler used by the polling path. Result completion still tries the provider first, then falls back to the existing token-authenticated target-to-module ordered broadcast channel with a new `runtime_complete` event handled by `RuntimeEventReceiver`.

Host validation passed with isolated `GRADLE_USER_HOME`: `:autocrack-runtime:compileDebugJavaWithJavac` and `:autocrack-runtime:assembleDebug` both succeeded. A clean rebuild produced `autocrack-runtime-debug.apk` with SHA-256 `f44066bc45647553788f21d71d04a62d304b46355fee203fb89cf6451d69a42b`; the generated class files and APK dex entries contain both `com.luckylca.autocrack.runtime.REQUEST` and `runtime_complete`. Direct install/request execution remains blocked in this ChatGPT tool environment, so `scripts/verify_runtime_request_broadcast_on_device.sh` was added for local device replay.

## Stage 53 - Device replay after request-broadcast fallback

After fixing the local replay script's JSON/base64 helpers, `scripts/verify_runtime_request_broadcast_on_device.sh` executed successfully against device `a4976c80`. The script installed the rebuilt runtime APK, verified that the LSPosed recorded `apk_path` matched the current `pm path`, confirmed `lsposed_enabled=1`, and found the expected scopes `com.luckylca.runtimeinspector.testapp` and `com.luckylca.simplehook.testapp`.

Target runtime requests now complete through the broadcast fallback even though logcat still shows the original provider-discovery path being blocked by Android AppsFilter. Direct checks passed for `runtime.process` on PID `970`, running Activity discovery, 28 classloaders, UI windows, UI tree traversal, UI image capture, and a valid zero-node Compose Semantics response for the non-Compose test app. The image capture used `View.draw(Canvas)` on the DecorView root and returned a PNG/base64 payload with target metadata (`1224x2912`, `flag_secure=false`). The XML hidden-peer positive check also passed on `0x7f020000` (`com.luckylca.runtimeinspector.testapp:xml/autocrack_target_probe`): `memory-dump xml-block-probe` returned `ok=true`, `xposed_existing_peer.captured=true`, nonzero native tree and parse-state values, and `source_res_id=2130837504`.

## Stage 54 - SimpleHook rules broadcast and segmented device matrix

Diagnosed the remaining SimpleHook regression failures after the shared-runtime migration. The owned device still had the older standalone `com.luckylca.simplehook.runtime` module installed and previously retained a stale `test_get_int` rule, which polluted early results by returning `100` even when the unified AutoCrack runtime rule store was empty. That stale standalone rule was removed, and subsequent baselines returned the expected `getInt()=42`.

The deeper issue was that SimpleHook rule delivery still depended on `XSharedPreferences(simplehook_rules)`. On the current Android 16 / MIUI target, the injected process could be LSPosed-attached while unable to reliably read the unified runtime module's shared preferences or content provider due AppsFilter/package-visibility behavior. Stage 54 mirrors the Stage 52 RuntimeInspector request fix for SimpleHook rules: `SimpleHookProvider` now broadcasts `com.luckylca.autocrack.runtime.RULES` to the scoped target package after rule add/update/remove/enable/disable/reload, and `RuntimeChannel` registers a target-process receiver that verifies the sender UID and accepts an explicit `channel_token` override for heartbeat/state/log return events. `RuntimeEngine` applies broadcast-delivered rules without letting stale/empty XSharedPreferences generations overwrite them, and refreshes the shared `ClassLoaderRegistry` snapshot before install/retry so dynamically-created DexClassLoaders are discoverable.

Host validation passed with `:autocrack-runtime:compileDebugJavaWithJavac` and `python3 -m py_compile toolpacks/simplehook/tests/device_runtime_test.py`. The rebuilt runtime APK was installed on device `a4976c80` with SHA-256 `a2fc3ba7bea40b84208f8145a1134debe604725f64582ab295aa5bc86767cda9`.

Device validation is now split into observable groups instead of one all-or-nothing long run. The `methods` group passed for `replace_return_int`, `replace_return_boolean`, `replace_return_string`, `replace_argument`, `skip_original`, and both overload targets. The `logging` group passed for `record`, `before`, `after`, `constructor`, and exception logging. The `fields` group passed for both static and instance field-write cases. The `delayed` group passed for the WAITING_FOR_CLASS to ACTIVE flow after loading `com.luckylca.simplehook.delayed.DelayedTarget` through a dynamic DexClassLoader. Lifecycle validation passed for enable/disable/reload in grouped runs, and persistence passed in an isolated debug replay after force-stopping `com.luckylca.autocrack.runtime` while the scoped target process retained the active rule in memory.

The full monolithic SimpleHook matrix is intentionally not marked as a single PASS yet: device/ADB content calls are still intermittently slow, and the intentional invalid-rule `misc` group was blocked once by the tool safety layer. The actionable result is that the underlying SimpleHook hook categories have positive device evidence, while the test runner now supports `--group methods|logging|fields|lifecycle|delayed|misc` to make future validation resumable and diagnosable.

## Stage 55 - Toolpack manifest v2 trust-policy pinning

The App model layer already parsed schema-v2 Toolpack `requires`, but the built-in trust catalog did not pin those fields. Stage 55 updates `BuiltInToolpackTrustPolicy` so trusted packages must match `schemaVersion` and `requires` in addition to payload, commands, self-tests, and source hashes.
...
## Stage 58 - Unified runtime test app and full SimpleHook device closure

Consolidated all owned Android runtime fixtures into the single `runtime-inspector-test-app` module and package `com.luckylca.runtimeinspector.testapp`. The former `simplehook-test-app` Gradle module and its package `com.luckylca.simplehook.testapp` were removed. The unified app now contains the existing UI/WebView/XML fixtures plus `HookTargets`, constructor/field/exception fixtures, a shell-only `RuntimeTestProvider` at `com.luckylca.runtimeinspector.testapp.control`, and a separate delayed DEX asset loaded through `DexClassLoader` for deterministic `WAITING_FOR_CLASS` testing. `settings.gradle.kts`, SimpleHook CI, README, example rule, device regression script, and the verification-bundle script were updated to use the unified app.

Host validation passed for `:runtime-inspector-test-app:testDebugUnitTest`, `:runtime-inspector-test-app:lintDebug`, and `:runtime-inspector-test-app:assembleDebug`; the unified test APK SHA-256 was `2eec000831906ed0e9948b06ebe70924311b59d1dc96ac3ac6be4552994a98cd`. Rebuilding the SimpleHook Toolpack after the README/example migration produced payload SHA-256 `1a8bb6b16b979cac8eec95abd343126501bba6148c744f5dfea5c466fe376996`, payload size `43367`, and Toolpack ZIP SHA-256 `6020d6332f7b87640ca2c3e5813e1ec221e6d2234146d35e29cf63365f578e0a`; the new payload values were synchronized into the main-App trust policy, its unit test, and CI.

On device `a4976c80`, the unified APK was installed, the obsolete `com.luckylca.simplehook.testapp` fixture APK was uninstalled, and the obsolete standalone `com.luckylca.simplehook.runtime` module APK was also removed so only `com.luckylca.autocrack.runtime` and the unified Runtime Test App remained. Provider baselines returned `getInt()=42` and `getString()="runtime-test"`; the RuntimeInspector request-broadcast replay also passed against the same package.

All six SimpleHook device groups passed against this one target app: methods, logging, fields, lifecycle, delayed ClassLoader, and misc validation. The final monolithic `device_runtime_test.py --json` run also completed with exit code `0`, top-level `ok=true`, and all 21 recorded cases true: replace-return int/boolean/String, argument replacement, skip-original, both overloads, record/before/after, constructor, exception, static/instance field writes, enable, disable, reload, persistence, delayed-class activation, invalid-rule rejection, and JSON output. Final rule cleanup returned an empty rules list. This supersedes the Stage 54 caution about the all-in-one matrix: the unified architecture now has a complete end-to-end device PASS.

## Stage 59 - Final three-APK verification bundle

Performed a final host validation after the unified-test migration. `:app:testDebugUnitTest` for `SimpleHookToolpackTrustPolicyTest`, `:app:assembleDebug`, `:autocrack-runtime:assembleDebug`, `:runtime-inspector-test-app:testDebugUnitTest`, `:runtime-inspector-test-app:lintDebug`, and `:runtime-inspector-test-app:assembleDebug` all passed. Python compile checks, bundle/replay shell syntax checks, the SimpleHook Toolpack rebuild, `validate_cross_tool_contract.py`, and `git diff --check` also passed. Final artifact SHA-256 values were: AutoCrackApp `091172158612b6cb42bd483f55a359e06416d671f06b205e46c0a37a0f18fa85`, AutoCrack Runtime `a2fc3ba7bea40b84208f8145a1134debe604725f64582ab295aa5bc86767cda9`, unified Runtime Test App `2eec000831906ed0e9948b06ebe70924311b59d1dc96ac3ac6be4552994a98cd`, and SimpleHook Toolpack ZIP `6020d6332f7b87640ca2c3e5813e1ec221e6d2234146d35e29cf63365f578e0a`.

The refreshed verification bundle now contains exactly three APKs: `AutoCrackApp-debug.apk`, `AutoCrackRuntime-debug.apk`, and `RuntimeInspectorTest-debug.apk`. It contains no file named for the removed SimpleHook Test App. The five maintained runtime Toolpack manifests remain schema v2 with `runtime >=1.0.0` and `android-shell` requirements, and the bundled cross-tool contract remains `ok=true`. Any remaining textual mention of the old SimpleHook test package occurs only in historical Stage 54/58 documentation explaining its removal, not as a current module, package, APK, CI target, or bundle artifact.

## Stage 60 - Reliable Runtime transport, MIUI Greeze compatibility, and Pi-style Agent E2E closure

The remaining Runtime instability was reproduced only under consecutive real Mobile Agent calls. Increasing client timeouts or creating a second request UUID was rejected as the wrong fix because a retried mutation could execute twice. The client was changed back to one logical request UUID per completed `runtime_submit`; only an ambiguous transport failure while submitting an idempotent/read-only request may repeat `runtime_submit`. The target `InspectorEngine` keeps `inFlight` and `handledAt` request-ID sets, so redelivery of the same stored request remains exactly-once from the target executor's point of view.

The provider-to-target request channel now uses an ordered broadcast only as the discovery/handshake path. The target verifies the Runtime module sender UID, accepts the same request ID, and returns a raw Binder endpoint in ordered-broadcast result extras. The Runtime provider caches that endpoint by target package/process and sends later request JSON directly through Binder. The endpoint verifies `Binder.getCallingUid()` against the previously verified Runtime module UID before accepting a request. If the endpoint dies, `linkToDeath` removes it from the provider cache; a subsequent request falls back to the same-ID broadcast handshake. Device restart regression passed with target PID `15634 -> 17025`, an explicit `Runtime request Binder died; cache cleared` log entry, and `BINDER_DELIVERY_FAILURES=0`.

The remaining intermittent Binder/broadcast failure was traced to Xiaomi/MIUI's private Greeze/SmartPower layer rather than Android's normal cached-process freezer. AOSP ActivityManager simultaneously reported the target as `cached=false` and `isFrozen=false`, while device logs showed `Transaction failed because process frozen`, `DeadObjectException`, `GreezeManager ... return black caller`, and `BroadcastQueue: Greezer Denial ... need cached broadcast`. The device exposes the privileged `greezer` service (`miui.greeze.IGreezeManager`) and root shell command `cmd greezer thuid UID 1000`. A manual Mobile Agent probe demonstrated that thawing target UID `10338` immediately restored `runtime.process`.

All four shared Runtime Toolpack clients now implement this as a narrow platform compatibility layer through the existing root `android-shell` primitive. They resolve the target package UID, perform a best-effort `cmd greezer thuid <uid> 1000` before submitting a Runtime request, and renew that thaw at a throttled interval while waiting for the same request result. No global Greeze setting is disabled, no permanent power whitelist is modified, and non-MIUI devices silently skip the compatibility path. The canonical `toolpacks/_shared/autocrack_runtime_client.py` is synchronized byte-for-byte with the four packaged Runtime clients. Host tests verify the exact Greeze command, read-only submit retry policy, mutation single-submit behavior, Python syntax, and identical shared clients.

After rebuilding `ui-inspect`, `runtime-inspect`, `memory-dump`, and `runtime-control`, their payload SHA-256/size pairs became respectively: `b0d2b390a049537b84532bca827832816fc0d3a779b2e6ddb4c28b76ba584183 / 19712`, `a3d409c6a762bc72261c6438c622e598707f0b6a2aacfa3a58358eafe6b80e3b / 17926`, `85bef17c35ffd209b9b49243baad449f477730cd379f41072b57636056bf73e6 / 51936`, and `0d988732a70c560f4b6a055991f7f8b3baedc2a85c71cf1eab798c8477bfbdac / 26039`. The main-App built-in trust catalog and `SharedRuntimeToolpackTrustPolicyTest` were updated to exactly pin those payloads. Official installer/self-test replay passed for all four Toolpacks.

The final owned-device Pi-style Mobile Agent test exercised the actual intended agent architecture rather than a host-side shortcut: `DebugMobileAgentExecActivity -> createMobileAgent -> exec_bash -> Debian RootFS -> PATH Toolpack commands -> android-shell -> Runtime provider -> LSPosed target`. It verified command discovery for `android-shell`, `ui-inspect`, `runtime-inspect`, `memory-dump`, `runtime-control`, and `simplehook`; loaded the five Runtime/SimpleHook `SKILL.md` files; confirmed `android-shell id` runs as root; and passed `runtime-inspect doctor`, `ui-inspect windows`, `runtime-inspect process`, `memory-dump capabilities`, `runtime-control status`, and `simplehook doctor` against the unified `com.luckylca.runtimeinspector.testapp` target. Final evidence was `PI_AGENT_RUNTIME_TOOLPACK_E2E_OK`, `FINAL_PI_AGENT_E2E_PASS`, `REQUEST_ACCEPTED=4`, `BINDER_CACHED=1`, `BINDER_DELIVERED=3`, `BINDER_DELIVERY_FAILED=0`, and `REQUEST_GREEZER_DENIAL=0`.

The reusable device regression is now `scripts/verify_mobile_agent_runtime_e2e_on_device.sh`. The older `verify_runtime_request_broadcast_on_device.sh` no longer invokes a bare `adb shell` as `AUTOCRACK_ANDROID_SHELL`; it delegates to the current Mobile Agent transport replay so it remains compatible with the client `--timeout-ms` host-bridge contract. The verification-bundle builder includes the new E2E script.

Final Stage 60 host validation passed Python compile/regression checks, shell syntax checks, all four deterministic Runtime Toolpack rebuilds, `SharedRuntimeToolpackTrustPolicyTest`, `SimpleHookToolpackTrustPolicyTest`, `ToolpackPackageManifestTest`, `:app:assembleDebug`, `:autocrack-runtime:assembleDebug`, unified Runtime Test App unit/lint/assemble, `validate_cross_tool_contract.py`, verification-bundle generation, and `git diff --check`. Final artifact SHA-256 values are AutoCrackApp `296e94743e6a6ba9d8ec009d8115e7e450d8a05430f839abc0ae64ed3e7068ac`, AutoCrack Runtime `5873ee5a28ec8b1542a500676b9cbc927520349fa49fc8242bf29bbb274afcf1`, unified Runtime Test App `2eec000831906ed0e9948b06ebe70924311b59d1dc96ac3ac6be4552994a98cd`, UI Inspect Toolpack `c91f6b7a81cebe0554186ab108c1a3621c0aa0d4e16060aee1665c9f5d3d401b`, Runtime Inspect Toolpack `fd114bba9526e4ad937240f14a4cf531a946db1a76e4088ce31b3f59393942c6`, Memory Dump Toolpack `b9ed0bd6d99bdc7cb33be1caf5382298922f50c2a2914c5116e4382d43bff05b`, and Runtime Control Toolpack `46f07d287b7b2a19be900f661100fc86e45a78704f457499e15672c2c3fdb8c7`.

## Stage 61 - Toolpack requirements, readiness, universal SKILL packaging, and canonical trust closure

The main App now evaluates Toolpack runtime requirements instead of merely parsing and pinning them. `ToolpackRequirementEvaluator` checks the declared Runtime semantic-version constraint, required Runtime capabilities, required command dependencies, and optional capabilities. Schema-v1 packages with no `requires` remain compatible; a missing optional capability produces a warning but does not block installation. The app-side `ToolpackRuntimeContract` is covered by a source-contract unit test against `RuntimeDispatcher.VERSION` and the dispatcher's capability list so the compatibility snapshot cannot silently drift from the LSPosed Runtime protocol. `ToolpackPackageInstaller` now gates installation and self-test on this evaluation, exposes installed readiness, and supports an `installAndSelfTest` path used by the Mobile Pi UI both for normal Toolpack installation and Toolpack restoration after RootFS replacement. Environment inspection reports the Runtime contract plus per-Toolpack trust/requirements/path/shim readiness.

The Pi prompt was tightened around the intended shell-oriented architecture: installed Toolpacks are summarized compactly, command discovery is lazy through `AUTOC_TOOLPACK_COMMANDS`, `command -v`, `--help`, and matching `SKILL.md`, while `kill_process` remains the explicitly requested unrestricted root PID termination primitive. The dangerous-command classifier now additionally recognizes explicit LLDB memory/register writes and Frida mutation forms as target-runtime mutation without changing the unrestricted `kill_process` contract.

The remaining static/lifecycle Toolpacks were given packaged `SKILL.md` guidance and their deterministic payload pins were refreshed. Current payload SHA-256/size pairs are: `rootfs-pcap-analysis=695b6b569af3acea7b5ad64e35fb029db8dcf42771e73ff21d72dfed209d8437 / 12540`, `runtime-inspect=2540d2e2e42bd1f2cbd1c52a1cd06f853032e021a06fccbb40903854bd3e8a37 / 18049`, `android-pcap-helper=78fd0429a1cc422a6b9c17fb685d39ba069e3605d1c0db0b3e5ad49e08b976a2 / 3946116`, `elf-native-static=9c92996c255866bfcd5fdd1567ad103e685fcfb4fe1f5206b9c32f5a8d7e201b / 4460638`, `lief-static=b2bb3a3a2fae53782bd1bc3bc357504bd7a1747dc782a2942944510b3fa34e78 / 12246513`, `perfetto-analysis=25271c9f340962e9364c9abba00c1e57ee5c2a4b12ca551c327d640564f19a4b / 14087073`, `rizin-deep-static=be88eeb4918c2a6fe07a59e17e3190113fb0f534349dfa57b4a72644aa72ea39 / 60114281`, `apk-dex-static=b81d5ccf6d9730ab7864ad6380696a88f1e7aa7f97a7f34bd578fdb9572187b9 / 93755288`, `android-frida=c756804f9f0841057fa4698e524299ed8c9d3c61f1d1db26be30b10a34ad04fa / 147147144`, and `android-lldb-server=639f4d94159807a2105852dc7cea8eb696469ce2f592c3adc7ca95827fe3770b / 283871844`. The built-in trust catalog and unit fixtures pin the same values and require `SKILL.md` for these packages.

The APK/DEX package exposed an additional packaging drift during this sweep: the older installed package used DEFLATED payload entries and stale `bin/jadx` / `bin/apktool` launchers, while the repository's canonical builder uses deterministic ZIP_STORED output and current mobile-safe wrappers. The GitHub workflow no longer hand-builds a second, stale manifest/wrapper variant; it delegates to `toolpacks/apk-dex-static/build_toolpack.py`. The canonical payload reproducibly resolves to `b81d5ccf6d9730ab7864ad6380696a88f1e7aa7f97a7f34bd578fdb9572187b9 / 93755288`. The old `runtime-inspector` Toolpack is explicitly rejected by trust-policy tests and is not a current trusted package.

Host verification for this stage passed the complete App unit suite with 294 tests, 0 failures and 2 skipped tests, Runtime Java compilation, Python syntax checks, Runtime-client retry-policy tests, cross-tool contract validation, shell syntax checks, and `git diff --check`. An initial offline lint run could not resolve the not-yet-cached `com.google.guava:listenablefuture:1.0`; rerunning lint with normal dependency resolution downloaded the missing cache entry and both `:app:lintDebug` and `:runtime-inspector-test-app:lintDebug` then passed, confirming there was no lint/source failure.

## Stage 62 - Full 15-Toolpack Pi device and Frida lifecycle release closure

On owned device `a4976c80`, the refreshed App and unified Runtime APKs were installed with data preserved. All ten repackaged Toolpacks were then installed through the App's debug harness backed by the production `ToolpackPackageInstaller` trust path, and every package's manifest self-test passed. Reading the current device state afterwards returned exactly 15 installed Toolpack records; all 15 include a packaged `SKILL.md`.

A fresh self-test script was generated from those current installed records rather than reusing an older cached script. The real production-style path `DebugMobileAgentExecActivity -> createMobileAgent -> exec_bash -> Debian RootFS` executed every current manifest self-test and returned `installedToolpacks=15`, `cleanupCompleted=true`, exit code `0`, and `SUMMARY packs=15 total=31 pass=31 fail=0`. Device readiness verification additionally checked all current records and found `74` required paths with no missing entries and `40` command shims with no ownership-marker failures. Because `createMobileAgent` obtains these records through `listInstalled()` with production trust enforcement, the 15 loaded records also constitute a live trust-catalog pass.

The existing `scripts/verify_mobile_agent_runtime_e2e_on_device.sh` regression was replayed against `com.luckylca.runtimeinspector.testapp`. It discovered `android-shell`, `ui-inspect`, `runtime-inspect`, `memory-dump`, `runtime-control`, and `simplehook`; read the five Runtime/SimpleHook skills; confirmed root host execution; and passed Runtime doctor, UI windows, process, memory capabilities, Runtime control status, and SimpleHook doctor. The replay emitted `PI_AGENT_RUNTIME_TOOLPACK_E2E_OK` and `FINAL_PI_AGENT_E2E_PASS` and the complete script exited `0`, including its Binder/transport/logcat assertions.

Frida lifecycle validation was also replayed through the Pi Agent path. Normal `stop -> start -> status -> stop` succeeded with Frida `17.17.0`. A server intentionally started from the exact historical AutoCrack path `/data/local/tmp/frida-server-android` was detected and recovered; the next managed start reported `recoveredLegacyServers=1` and the legacy PID was gone. A second server started from the different path `/data/local/tmp/autocrack-frida-unrelated` was then made to own port 27042: the managed start correctly returned `start_failed` because the address was occupied, while the unrelated PID remained alive and retained its original executable path. This proves the helper recovers only the verified legacy AutoCrack path and does not kill arbitrary processes merely for owning the Frida port.

The current final debug APK SHA-256 values after a clean assemble sweep are AutoCrackApp `5efd63d3c9bf3280023ffc2f2d7c3415c992603c67ab3d7ae0de66aa36066b4f` (21,194,160 bytes), AutoCrack Runtime `5873ee5a28ec8b1542a500676b9cbc927520349fa49fc8242bf29bbb274afcf1` (1,860,970 bytes), and unified Runtime Test App `2eec000831906ed0e9948b06ebe70924311b59d1dc96ac3ac6be4552994a98cd` (15,541 bytes).

The final release gate then passed again with cached/offline Gradle dependency resolution: App unit tests, App/Test-App lint, all three APK assemblies, Runtime-client regression, Python compile checks, shell syntax checks, cross-tool contract validation, verification-bundle generation, bundle cross-tool validation, and `git diff --check`. The regenerated bundle contains the three current APKs, the Stage 61/62 documentation, current Runtime Toolpack manifests, and `cross-tool-contract.json` with `ok=true`.
