...
Added `memory.dex.art_dump` and `memory-dump dex-art-dump`. The new path does not accept arbitrary addresses. It internally reruns the Stage 37 high-confidence ART candidate discovery, selects a validated `data_begin + file_size` candidate, re-reads exactly that range, and revalidates DEX magic, `file_size`, `header_size`, endian tag, and the previously observed memory SHA-256 before permitting raw-byte export. The CLI writes the returned base64 payload through the existing `--output` materialization path. Candidates larger than the 4 MiB runtime inline cap are rejected; unbounded/chunked streaming is still not implemented.
