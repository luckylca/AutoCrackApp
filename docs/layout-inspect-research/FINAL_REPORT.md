...
The Java implementation and focused Gradle build passed. The updated runtime APK also installed successfully on the API 36 device. A direct provider-self invocation of the ART fingerprint path was blocked by the current tool safety layer before execution, so Stage 39 is not recorded as a device PASS. No attempt was made to bypass that block. This stage therefore remains host/build validated with device assertion blocked, while the already device-proven Stage 37/38 ART header and table reconstruction remains unchanged.
