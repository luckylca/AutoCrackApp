...
API 36 provider-self validation passed on `autocrack_runtime_probe.xml` (340 bytes): the platform native tree and parser were both created, the isolated native sequence returned `[2, 3, 1]` (`START_TAG`, `END_TAG`, `END_DOCUMENT`), `end_document_reached=true`, and both native peers completed their destroy path. The ordinary Java parser simultaneously exposed the expected document/start/end events. This proves the recovered runtime table can drive a real platform `ResXMLTree` lifecycle, while extraction of an already-existing framework object's hidden `mNative` / `mParseState` still remains unclaimed.
...
API 36 provider-self validation passed on `autocrack_runtime_probe.xml`. The native START_TAG reported `name_index=3`, `source_res_id=0x7f010000`, two attributes, and typed metadata matching the Java-side resource: Android `name` resolved to resource ID `0x01010003`, data type `TYPE_STRING` with string index 4, while Android `version` resolved to `0x01010519`, data type `TYPE_INT_DEC`, data `1`. The replay again reached END_DOCUMENT and destroyed both peers. Existing-object hidden-peer extraction remains the remaining XmlBlock-native gap.
