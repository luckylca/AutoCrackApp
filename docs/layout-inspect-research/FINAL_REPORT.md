...
Provider-self validation on API 36 passed the required negative behavior: the module app is intentionally excluded from Xposed injection, so `xposed_existing_peer.captured=false` and `xposed_required=true` were returned while the Stage 44-46 native replay/string checks still passed. A dedicated target-app XML fixture (`0x7f020000`) built and installed successfully and the app cold-started, but the target runtime request still timed out under the previously recorded LSPosed module-path refresh limitation. Therefore the Xposed hidden-peer capture implementation is host/provider-negative validated and remains TARGET-PENDING for a positive target capture until the user's normal LSPosed refresh/reboot path loads the current runtime APK. No LSPosed database mutation or forced reboot was performed.
