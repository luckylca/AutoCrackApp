...
| Existing XmlBlock hidden-peer Xposed capture | PASS(host build + provider-self negative + Toolpack), TARGET-PENDING | Target-only hook records `XmlBlock.mNative` and `Parser.mParseState` through Xposed hidden-field access after `XmlBlock.newParser(int)`, with no ART offset guessing. Focused Gradle rebuild passed after the capability updates, `memory-dump xml-block-probe --help` exposes the native probe flags, and the memory-dump Toolpack rebuild produced SHA-256 `26537ea49f9372c5054375a13774d08575e382c4e869a96f30ec26395685333c`. Provider-self correctly returned `captured=false/xposed_required=true`; positive target capture remains pending LSPosed module-path refresh and an allowed target-device validation run. |
...
| SimpleHook Toolpack package | PASS(host) | Rebuilt `simplehook-toolpack-0.1.1.zip` after manifest-v2 alignment with Toolpack SHA-256 `b29b064496b18145a5b48b8da6e899c4d0b2dcc31d89bc71e6158be0ed4bb9a3` and payload SHA-256 `52de7ef3f08bc698300d7a4abd9163450d0b50356e01ab29210d5f8d6dffaa7b`. |
