...
| Existing XmlBlock hidden-peer Xposed capture | PASS(host build + provider-self negative), TARGET-PENDING | Target-only hook records `XmlBlock.mNative` and `Parser.mParseState` through Xposed hidden-field access after `XmlBlock.newParser(int)`, with no ART offset guessing. Provider-self correctly returned `captured=false/xposed_required=true`; target fixture `0x7f020000` built/installed/cold-started, but target request timed out under the existing LSPosed module-path refresh limitation. |
