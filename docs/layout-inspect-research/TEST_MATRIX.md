...
| API 36 XmlBlock hidden peer mapping | PASS(device DEX/native binary analysis + runtime build), RUNTIME TABLE ASSERTION BLOCKED | Provider reflection/JNI lookup hides `mNative`/`mParseState`, but the device's own `framework.jar/classes.dex` proves `XmlBlock.mNative:J` and `Parser.mParseState:J` exist with hidden-API flags; `newParser` calls `nativeCreateParseState` and `Parser.next` calls `nativeNext`. Device `libandroid_runtime.so` exposes the XmlBlock registration entry and a 22-entry JNINativeMethod table matching the DEX native signatures. Runtime offset-free table discovery builds successfully; its provider-self assertion was blocked by tool safety and is not marked PASS. |
