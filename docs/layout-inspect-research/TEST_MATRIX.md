...
| SimpleHook lifecycle group | PARTIAL(device) | Grouped run passed `enable`, `disable`, and `reload`; isolated persistence replay also passed after force-stopping `com.luckylca.autocrack.runtime`. One grouped lifecycle run still reported `persistence=false`, so persistence is recorded as positive isolated evidence, not full grouped-matrix PASS. |
| SimpleHook monolithic matrix | NOT CLAIMED | The old all-in-one matrix is no longer the source of truth because device/ADB content calls intermittently stall and hide progress. The script now supports grouped validation with per-step stderr progress for resumable evidence. |
