...
| ART DEX content fingerprint correlation | PASS(host build/install), DEVICE ASSERTION BLOCKED | Validated ART candidates now compute memory SHA-256/DEX signature and compare same-size originating APK `classes*.dex` entries without returning bytes. Runtime build/install passed; direct provider-self fingerprint assertion was blocked by tool safety before execution. |
