...
| Runtime XML | XML path UI | CONFIRMED surface | logical `Resources.getXml`, raw APK AXML decode, XmlBlock object/event/method probe, API 36 hidden-peer DEX mapping (`mNative`/`mParseState`), device-validated offset-free 22-entry JNI-backend discovery, and device-validated isolated native XmlBlock/ResXMLTree create/parse/destroy replay from APK AXML; extracting an existing runtime object's hidden native peer / direct peer-memory reconstruction remains gated |
