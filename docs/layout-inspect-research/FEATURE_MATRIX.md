...
| Dex dump | mCookie/mInternalCookie/DexCaches modes and `C1187N` offsets | CONFIRMED strategies | Java cookie strategy + file-backed fallback; API-gated ART `data_begin`/size header-map reconstruction and bounded strings/classes/fields/methods/class_data parsing are device-validated; content SHA-256/APK correlation is implemented but latest device assertion is safety-blocked; full raw byte reconstruction/export remains unsupported |
