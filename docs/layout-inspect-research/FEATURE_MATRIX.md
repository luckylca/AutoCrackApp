...
| Dex dump | mCookie/mInternalCookie/DexCaches modes and `C1187N` offsets | CONFIRMED strategies | Java cookie strategy + file-backed fallback; API-gated ART `data_begin`/size header-map reconstruction is now device-validated, while full byte reconstruction/export remains unsupported |
