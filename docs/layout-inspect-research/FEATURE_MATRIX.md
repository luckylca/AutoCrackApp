...
| Dex dump | mCookie/mInternalCookie/DexCaches modes and `C1187N` offsets | CONFIRMED strategies | Java cookie strategy + file-backed fallback; API-gated ART `data_begin`/size header-map reconstruction, bounded strings/classes/fields/methods/class_data parsing, SHA-256/APK correlation, and bounded validated raw dump (<=4 MiB) are device-validated; arbitrary/unbounded/chunked ART raw reconstruction remains unsupported |
