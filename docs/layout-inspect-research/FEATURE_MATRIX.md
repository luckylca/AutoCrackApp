...
| Runtime XML | XML path UI | CONFIRMED surface | logical `Resources.getXml`, raw APK AXML decode, XmlBlock object/event/method probe, API 36 hidden-peer DEX mapping (`mNative`/`mParseState`), and offset-free 22-entry XmlBlock JNI-backend table discovery implementation; extracting an existing runtime object's hidden native peer / direct ResXMLTree memory reconstruction remains gated |
