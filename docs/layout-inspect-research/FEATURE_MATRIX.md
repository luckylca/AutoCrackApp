...
| Runtime XML | XML path UI | CONFIRMED surface | logical `Resources.getXml`, raw APK AXML decode, XmlBlock object/event/method probe, API 36 hidden-peer DEX mapping, offset-free 22-entry JNI-backend discovery, isolated native `ResXMLTree` replay, native typed-attribute metadata, and readable native string-pool correlation back to the exact APK AXML; extracting an existing runtime object's hidden native peer / direct peer-memory reconstruction remains gated |
