...
| Dex dump | mCookie/mInternalCookie/DexCaches modes and `C1187N` offsets | CONFIRMED strategies | Java cookie strategy + file-backed fallback; API-gated ART `data_begin`/size header-map reconstruction, bounded tables, SHA-256/APK correlation, validated raw dump <=4 MiB, and token-bound chunk export up to 512 MiB are device-validated on the current API 36 layout; arbitrary layouts, >512 MiB candidates, and unreadable/non-contiguous ranges remain unsupported |
| Runtime XML | XML path UI | CONFIRMED surface | logical `Resources.getXml`, raw APK AXML decode, XmlBlock object/event/method probe, API 36 hidden-peer DEX mapping, offset-free 22-entry JNI-backend discovery, isolated native `ResXMLTree` replay, typed/readable native metadata, plus target-only Xposed `newParser(int)` capture for existing `mNative/mParseState` without ART offset guessing; target capture passed on the owned API 36 runtime-inspector test app after request-broadcast fallback, while direct arbitrary peer-memory reconstruction remains gated |
...
- `simplehook.rules.broadcast`: provider-to-target rule delivery for scoped processes. Rules no longer rely only on `XSharedPreferences`; the target receiver accepts verified AutoCrack Runtime broadcasts and uses the transmitted channel token for heartbeat/state/log replies. Device grouped replays passed for method hooks, logging, constructors, exception logging, field writes, lifecycle enable/disable/reload, and dynamic DexClassLoader delayed-class hooks; the monolithic all-in-one matrix remains intentionally unclaimed because of intermittent ADB/content-call stalls.
