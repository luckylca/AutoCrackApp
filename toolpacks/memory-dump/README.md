...
`memory-dump dex-art-pointer-probe --package PKG --max-pointers 32 --include-words --json` maps ART `mCookie`/`mInternalCookie` pointer values into `/proc/self/maps` and performs a bounded DEX magic/header neighborhood scan; `--include-words` adds a bounded 64-bit word table with map back-references and APK `classes*.dex` size correlation hints. `--try-layout-dex-header` is an explicit version-gated opt-in that pairs readable ART pointer words with candidate file-size words, deduplicates identical data addresses, and only emits a high-confidence `layout_dex_candidates` item when DEX `file_size`, `header_size`, endian tag, and map metadata validate. On the API 36 fixture this reconstructed DEX 038 header/map metadata directly from ART memory. `art_memory_header_reconstruction` may therefore be true while `art_memory_reconstruction` remains false; no DEX bytes are exported.
