...
`memory-dump xml-block-probe --package PKG 0x7f... --json` opens a runtime `XmlResourceParser`, reflects parser/`XmlBlock` field and method shape, emits a bounded XML event/attribute preview, and correlates the parser resource with its APK-backed AXML metadata (`size`, `compressed_size`, `crc`, `sha256`) without base64 data. Hidden-API filtering can hide real framework fields, so failed field lookup is not reported as field absence. Add `--native-backend` only when you explicitly want the loaded `libandroid_runtime.so` XmlBlock `JNINativeMethod` table discovery. Add `--native-replay` to feed that resource's own APK-backed binary AXML into a newly created platform XmlBlock/ResXMLTree, create a parse state, advance a bounded event sequence, return native typed resource/data metadata, and resolve the native namespace/name/text/string-value indices through that exact AXML string pool into readable strings before destroying both native peers. The replay is isolated and does not claim to recover an existing runtime object's hidden `mNative`/`mParseState` peer.

When the current AutoCrack Runtime is actually injected into the target by Xposed/LSPosed, the same probe also checks `xposed_existing_peer`. The Xposed bootstrap hooks `XmlBlock.newParser(int)` and weakly records that exact parser's hidden `mParseState` and owning block's `mNative`; this avoids hard-coded ART object offsets. If the current module is not loaded into the target, the result stays `captured=false` with `xposed_required=true` rather than inventing a peer.
...
`memory-dump dex-art-pointer-probe --package PKG --max-pointers 32 --include-words --json` maps ART `mCookie`/`mInternalCookie` pointer values into `/proc/self/maps` and performs a bounded DEX magic/header neighborhood scan. `--try-layout-dex-header` is an explicit version-gated opt-in that pairs readable ART pointer words with candidate file-size words, deduplicates identical data addresses, and only accepts a high-confidence candidate after DEX `file_size`, `header_size`, endian tag, and map metadata validate. `--try-layout-dex-tables` implies the header/word probe and additionally parses bounded strings, classes, fields, methods, and class_data directly from that validated ART memory candidate; tune it with `--layout-table-limit`, `--layout-member-limit`, and `--layout-filter`. These probe modes expose metadata only and do not return raw DEX bytes themselves. Use the validated `dex-art-dump` or `dex-art-export` paths when raw bytes are required.

`memory-dump dex-art-dump --package PKG --output recovered.dex --json` performs the same high-confidence ART candidate discovery, selects a validated candidate with `--candidate-index`, re-reads it from `data_begin`, and revalidates DEX magic, `file_size`, `header_size`, endian tag, and the previously observed memory SHA-256 before returning raw bytes. This inline dump path is deliberately bounded by the 4 MiB runtime cap; arbitrary pointers and unvalidated candidates are rejected. For larger validated candidates, use `dex-art-export`, whose token-bound chunk path is capped at 512 MiB.

`memory-dump dex-art-export --package PKG --output recovered.dex --json` is the streaming path for larger validated ART DEX mappings. It opens a short-lived target-process export token bound to a high-confidence `data_begin + file_size + DEX header/signature`, fetches at most 512 KiB per Binder response, revalidates the DEX header before every chunk, verifies each chunk SHA-256 in the CLI, writes sequentially to the output file, computes the final file SHA-256 locally, and closes the token. Candidate discovery is capped at 512 MiB and requires a contiguous readable mapping range. The command uses a 20-second default runtime timeout because ART candidate discovery can exceed the shared 5-second default; use `--timeout` to override it for slower targets.
