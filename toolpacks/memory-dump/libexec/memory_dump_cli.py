...
def parser():
    ...
    md=sub.add_parser("module-dump"); add_target(md); md.add_argument("path")
    dl=sub.add_parser("dex-list"); add_target(dl); dl.add_argument("--loader", help="optional ClassLoader handle returned by runtime-inspect classloaders")
    dd=sub.add_parser("dex-dump"); add_target(dd); dd.add_argument("dex_handle")
    ...
    ap=sub.add_parser("assets-pull"); add_target(ap); ap.add_argument("path")
...
def execute(a):
    ...
    if a.command=="module-dump": return runtime_request(target_payload(a,"memory.module.dump", path=a.path), a.timeout)
    if a.command=="dex-list": return runtime_request(target_payload(a,"memory.dex.list", loader=a.loader), a.timeout)
    if a.command=="dex-dump": return runtime_request(target_payload(a,"memory.dex.dump", dex=a.dex_handle), a.timeout)
    ...
    if a.command=="assets-pull": return runtime_request(target_payload(a,"memory.assets.pull", path=a.path), a.timeout)
