...
def parser():
    ...
    xbp=sub.add_parser("xml-block-probe"); add_target(xbp); xbp.add_argument("resource_id", type=lambda x:int(x,0)); xbp.add_argument("--max-events", type=int, default=64); xbp.add_argument("--max-attributes", type=int, default=64)
...
def execute(a):
    ...
    if a.command=="xml-block-probe": return runtime_request(target_payload(a,"memory.xml.block_probe", resource_id=a.resource_id, max_events=a.max_events, max_attributes=a.max_attributes), a.timeout)
