...
def parser():
    ...
    xbp=sub.add_parser("xml-block-probe"); add_target(xbp); xbp.add_argument("resource_id", type=lambda x:int(x,0)); xbp.add_argument("--max-events", type=int, default=64); xbp.add_argument("--max-attributes", type=int, default=64); xbp.add_argument("--max-source-bytes", type=int, default=4194304)
...
def execute(a):
    ...
    if a.command=="dex-art-pointer-probe": return runtime_request(target_payload(a,"memory.dex.art_pointer_probe", loader=a.loader, include_class_count=a.class_count, max_dex=a.max_dex, max_pointers=a.max_pointers, window_bytes=a.window_bytes, scan_back_bytes=a.scan_back_bytes, include_words=a.include_words, word_count=a.word_count, try_layout_dex_header=a.try_layout_dex_header, include_context_loader=not a.no_context_loader), a.timeout)
    ...
    if a.command=="xml-block-probe": return runtime_request(target_payload(a,"memory.xml.block_probe", resource_id=a.resource_id, max_events=a.max_events, max_attributes=a.max_attributes, max_source_bytes=a.max_source_bytes), a.timeout)
