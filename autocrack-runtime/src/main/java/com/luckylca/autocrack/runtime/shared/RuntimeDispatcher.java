...
public final class RuntimeDispatcher {
    ...
    public static JSONObject capabilities() throws Exception {
        ...
        JSONArray partial = new JSONArray()
                ...
                .put(new JSONObject().put("capability","memory.dex.art_dump").put("reason","this inline command supports high-confidence validated ART candidates within the 4 MiB cap; larger validated candidates should use the token-bound memory.dex.art_export path up to 512 MiB"))
                .put(new JSONObject().put("capability","memory.dex.art_export.open").put("reason","session-based chunked export supports validated readable ART candidates up to 512 MiB; chunks are capped at 512 KiB and require a short-lived token"))
                ...
                .put(new JSONObject().put("capability","memory.xml.block_probe").put("reason","XmlResourceParser/XmlBlock object-shape probing, opt-in native backend table discovery, isolated APK-backed ResXMLTree replay, and target-only Xposed existing-peer capture are supported; direct arbitrary peer-memory reconstruction remains gated"))
